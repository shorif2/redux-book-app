...
const BookCart = ({ book, onUpdate }) => {
  const { name, author, price, rating, featured, id, thumbnail } = book;
    ...
    <div className="book-card">
      ...
      <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
        <div className="flex items-center justify-between">
          {featured ? (
            <span className={` badge-success lws-Badge `}>Feature</span>
          ) : (
            <span className={` badge-success lws-Badge `}></span>
          )}
          <div className="text-gray-500 space-x-2">
            <button onClick={() => onUpdate(book)} className="lws-edit">
              <svg
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
